...
import org.kimura.calc.dto.ConfigDto;
import org.kimura.calc.visual.NumericVisual;
...
public class App
{
	...
	protected	void	initApps(String[] args) {
		...
		try {
			InputStream is = new FileInputStream("calc.config");
			props.load(is);
	...
	protected void myMain(String[] args) {
		...
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					((NumericVisual)container.getComponent(NumericVisual.class)).setVisible(true);
				} catch (Exception e) {
					e.printStackTrace();
				}
			}
